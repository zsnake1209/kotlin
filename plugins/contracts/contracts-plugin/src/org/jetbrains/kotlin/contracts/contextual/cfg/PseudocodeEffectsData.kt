...
class PseudocodeEffectsData(
    ...
    fun computeResultingContexts(): Map<ContextFamily, List<Context>>? = resultingContexts

    private val resultingContexts: Map<ContextFamily, List<Context>>? by lazy { computeEffectsControlFlowInfo(pseudocode) }
    ...
    private fun ContractsContextsInfo.toMutableMap() = convertToMap()
        .mapValuesTo(mutableMapOf()) { (_, map) -> map.toMutableMap() }
}