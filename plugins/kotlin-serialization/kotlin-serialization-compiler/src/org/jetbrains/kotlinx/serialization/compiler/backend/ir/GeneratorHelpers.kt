...
interface IrBuilderExtension {
    ...
    private fun IrClass.declareSimpleFunctionWithExternalOverrides(descriptor: FunctionDescriptor, declare: Boolean): IrSimpleFunction {
        val functionSymbol = compilerContext.symbolTable.referenceSimpleFunction(descriptor)
        assert(functionSymbol.isBound || declare)

        if (declare) {
            IrFunctionImpl(startOffset, endOffset, SERIALIZABLE_PLUGIN_ORIGIN, functionSymbol, IrUninitializedType).also { f ->
                f.parent = this
                addMember(f)
                descriptor.returnType?.let { f.returnType = it.toIrType() }
                f.overriddenSymbols = descriptor.overriddenDescriptors.map {
                    compilerContext.symbolTable.referenceSimpleFunction(it.original)
                }
                f.createParameterDeclarations(null, true, true)
            }
        }

        return functionSymbol.owner
    }
    ...
    fun IrClass.contributeFunction(
        ...
        val f: IrSimpleFunction = declareSimpleFunctionWithExternalOverrides(descriptor, DescriptorUtils.isLocal(descriptor))
    ...
    fun IrClass.contributeConstructor(
        ...
        assert(ctorSymbol.isBound || declareNew)

        if (declareNew) {
            IrConstructorImpl(startOffset, endOffset, SERIALIZABLE_PLUGIN_ORIGIN, ctorSymbol, IrUninitializedType).also { ctor ->
                ctor.parent = this
                addMember(ctor)
                ctor.returnType = descriptor.returnType.toIrType()
                ctor.createParameterDeclarations(receiver = null, overwriteValueParameters = true, copyTypeParameters = true)
            }
        }
