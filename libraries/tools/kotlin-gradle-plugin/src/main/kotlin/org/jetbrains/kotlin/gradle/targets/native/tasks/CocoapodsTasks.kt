...
open class PodspecTask : DefaultTask() {
    ...
    @get:OutputFile
    internal val outputFileProvider: Provider<File> = project.provider { project.buildDir.resolve("$specName.podspec") }
    ...
    fun generate() {
        ...
        outputFileProvider.get().writeText(
            """
        ...
        logger.quiet(
            """
            Generated a podspec file at: ${outputFileProvider.get().absolutePath}.
            To include it in your Xcode project, add the following dependency snippet in your Podfile:

                pod '$specName', :path => '${outputFileProvider.get().parentFile.absolutePath}'
