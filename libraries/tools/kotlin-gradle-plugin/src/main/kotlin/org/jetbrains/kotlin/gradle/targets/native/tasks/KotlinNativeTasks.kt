...
abstract class AbstractKotlinNativeCompile<T : KotlinCommonToolOptions> : AbstractCompile() {
    ...
    @get:Internal
    abstract val compilation: Provider<KotlinNativeCompilation>
    ...
    val libraries: FileCollection by project.provider {
        project.files(compilation.map { it.compileDependencyFiles }).filterOutPublishableInteropLibs(project)
    }
...
open class KotlinNativeCompile : AbstractKotlinNativeCompile<KotlinCommonOptions>(), KotlinCompile<KotlinCommonOptions> {
    ...
    @get:Internal
    override val baseName: String by compilation.map { if (it.isMainCompilation) project.name else it.name }
    ...
    private val friendModule: FileCollection by compilation.map { compilationInstance ->
        project.files(
            compilationInstance.friendCompilations.map { compilationInstance.output.allOutputs },
    ...
    private inner class NativeCompileOptions : KotlinCommonOptions {
        ...
        override var freeCompilerArgs: List<String>
            get() = freeCompilerArgsImpl
            set(value) {
                compilation.get().extraOptsNoWarn = value.toMutableList()
            }
    }
