...
abstract class AbstractKotlinTarget (
    ...
    private fun createKotlinTargetComponent(): KotlinTargetComponent {
        ...
        return if (kotlinExtension.publishGradleMetadata) {
    ...
    override fun createUsageContexts(): Set<UsageContext> {
        val shouldExposeKotlinAttributes = (project.kotlinExtension as? KotlinMultiplatformExtension)?.publishGradleMetadata
        ...
        return listOfNotNull(
            KotlinPlatformUsageContext(
                ...
                apiElementsConfigurationName, shouldExposeKotlinAttributes
            ),
            if (compilations.getByName(KotlinCompilation.MAIN_COMPILATION_NAME) is KotlinCompilationToRunnableFiles) {
                KotlinPlatformUsageContext(
                    ...
                    runtimeElementsConfigurationName, shouldExposeKotlinAttributes
                )
            } else null
        ).toSet()
    }
