...
open class KotlinJsTargetConfigurator(kotlinPluginVersion: String) :
    ...
    KotlinTargetWithTestsConfigurator<KotlinJsCompilation, KotlinJsReportAggregatingTestRun, KotlinJsTarget> {
    ...
    override fun createTestRun(
        ...
        target.testRuns.matching { it.name == name }.whenObjectAdded {
            it.configureEachTestRun {
                // do not do anything with the aggregated test run, but ensure that they are created
            }
        }

        @Suppress("UNCHECKED_CAST")
        result.testTask = testTask.getTaskOrProvider() as TaskProvider<KotlinTestReport>
