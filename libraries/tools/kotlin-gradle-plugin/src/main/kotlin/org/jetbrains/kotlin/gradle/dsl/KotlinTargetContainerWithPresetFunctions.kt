...
import org.jetbrains.kotlin.gradle.plugin.mpp.KotlinBrowserJsTargetPreset
...
import org.jetbrains.kotlin.gradle.plugin.mpp.KotlinNodeJsTargetPreset
import org.jetbrains.kotlin.gradle.targets.js.KotlinBrowserJsTarget
import org.jetbrains.kotlin.gradle.targets.js.KotlinJsTarget
import org.jetbrains.kotlin.gradle.targets.js.KotlinNodeJsTarget
...
interface KotlinTargetContainerWithPresetFunctions : KotlinTargetsContainerWithPresets {
    ...
    fun nodeJs(
        name: String = "nodeJs",
        configure: KotlinNodeJsTarget.() -> Unit = { }
    ): KotlinNodeJsTarget =
        configureOrCreate(
            name,
            presets.getByName("nodeJs") as KotlinNodeJsTargetPreset,
            configure
        )

    fun nodeJs() = nodeJs("nodeJs") { }
    fun nodeJs(name: String) = nodeJs(name) { }
    fun nodeJs(name: String, configure: Closure<*>) = nodeJs(name) { ConfigureUtil.configure(configure, this) }
    fun nodeJs(configure: Closure<*>) = nodeJs { ConfigureUtil.configure(configure, this) }

    fun browserJs(
        name: String = "browserJs",
        configure: KotlinBrowserJsTarget.() -> Unit = { }
    ): KotlinBrowserJsTarget =
        configureOrCreate(
            name,
            presets.getByName("browserJs") as KotlinBrowserJsTargetPreset,
            configure
        )

    fun browserJs() = browserJs("browserJs") { }
    fun browserJs(name: String) = browserJs(name) { }
    fun browserJs(name: String, configure: Closure<*>) = browserJs(name) { ConfigureUtil.configure(configure, this) }
    fun browserJs(configure: Closure<*>) = browserJs { ConfigureUtil.configure(configure, this) }
