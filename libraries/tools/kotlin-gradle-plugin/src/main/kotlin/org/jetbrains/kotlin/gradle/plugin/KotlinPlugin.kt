...
abstract class AbstractAndroidProjectHandler<V>(private val kotlinConfigurationTools: KotlinConfigurationTools) {
    ...
    private fun addKotlinDependenciesToAndroidSourceSets(
        ...
        // In single-platform projects, the Kotlin compilations reference the Android plugin's configurations by the names, so don't do this
        if (kotlinAndroidTarget.disambiguationClassifier != null) {

            val androidSourceSetsByVariant: Map<V, List<AndroidSourceSet>> =
            ...
            val variantsBySourceSet: Map<AndroidSourceSet, List<V>> = mutableMapOf<AndroidSourceSet, MutableList<V>>().apply {
                androidSourceSetsByVariant.forEach { (variant, sourceSets) ->
            ...
            singleVariantSourceSets.forEach { (androidSourceSet, variant) ->
                val compilation = kotlinAndroidTarget.compilations.getByName(getVariantName(variant))
                addDependenciesToAndroidSourceSet(
                    androidSourceSet,
