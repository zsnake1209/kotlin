...
internal class Kotlin2JvmSourceSetProcessor(
    ...
    override fun doTargetSpecificProcessing() {
        ...
        project.afterEvaluate { project ->
            if (project != null) {
                ...
                if (project.plugins.hasPlugin("java-library") && sourceSetName == SourceSet.MAIN_SOURCE_SET_NAME) {
                    val classesDirectory = (kotlinAfterJavaTask ?: kotlinTask).run { javaOutputDir ?: destinationDir }
                    val classesProviderTask = syncOutputTask ?: kotlinAfterJavaTask ?: kotlinTask
    ...
    private fun registerKotlinOutputForJavaLibrary(outputDir: File, taskDependency: Task): Boolean {
        val configuration = project.configurations.findByName("apiElements")
                ?: return false
        ...
        checkedReflection({
            ...
            val variant = variants.findByName("classes")
                    ?: return false
            ...
            logger.kotlinDebug("Could not register Kotlin output of source set $sourceSetName for java-library: $reflectException")
            return false
        })
    }
}
