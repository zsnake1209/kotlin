...
val isTC = project.hasProperty("teamcity")
...
val cleanTestOut by task<Delete> {
    delete("$testDataDir/out", "$testDataDir/out-min")
...
val runMocha by task<NpmTask> {
    ...
    val target = if (isTC) "runOnTeamcity" else "test"
