        ...
        private fun checkLocalVariableTypes(cr: ClassReader, methodName: String, locals: List<LocalVariable>) {
            ...
            class BasicBlock(
                ...
                fun addInstruction(index: Int, opcode: Int) {
                    when (opcode) {
                        Opcodes.ILOAD -> localsInstructions.add(Load(index, "I"))
                        Opcodes.LLOAD -> localsInstructions.add(Load(index, "J"))
                        Opcodes.FLOAD -> localsInstructions.add(Load(index, "F"))
                        Opcodes.DLOAD -> localsInstructions.add(Load(index, "D"))
                        Opcodes.ALOAD -> localsInstructions.add(Load(index, "Ljava/lang/Object;"))
                        Opcodes.ISTORE -> localsInstructions.add(Store(index, "I"))
                        Opcodes.LSTORE -> localsInstructions.add(Store(index, "J"))
                        Opcodes.FSTORE -> localsInstructions.add(Store(index, "F"))
                        Opcodes.DSTORE -> localsInstructions.add(Store(index, "D"))
                        Opcodes.ASTORE -> localsInstructions.add(Store(index, "Ljava/lang/Object;"))
                ...
                private fun recordLocalTypesForParameters(access: Int, desc: String) {
                    ...
                    Type.getMethodType(desc).argumentTypes.forEach {
                        when (it) {
                            ...
                            Type.BYTE_TYPE,
                            Type.SHORT_TYPE,
                            Type.INT_TYPE -> localsAtEntry[parameterIndex++] = "I"

                            Type.FLOAT_TYPE-> localsAtEntry[parameterIndex++] = "F"

                            Type.DOUBLE_TYPE -> {
                                localsAtEntry[parameterIndex++] = "D"
                                // A double takes up two slots.
                                parameterIndex++
                            }
                            Type.LONG_TYPE -> {
                                localsAtEntry[parameterIndex++] = "J"
                                // A double takes up two slots.
                                parameterIndex++
                            }
                            else -> localsAtEntry[parameterIndex++] = "Ljava/lang/Object;"
                        }
                    }
                }
