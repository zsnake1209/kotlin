...
fun <T> bind(r: Option<T>): Option<T> {
    return if (r is Some) {
        ...
        (<!OI;TYPE_INFERENCE_FAILED_ON_SPECIAL_CONSTRUCT!>if<!> (true) <!OI;TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH!>None()<!> else <!DEBUG_INFO_SMARTCAST!>r<!>) checkType { <!NI;UNRESOLVED_REFERENCE_WRONG_RECEIVER, NI;DEBUG_INFO_UNRESOLVED_WITH_TARGET, OI;TYPE_MISMATCH!>_<!><Option<T>>() }
        // Works correctly
        if (true) None() else r
    }
    else r
