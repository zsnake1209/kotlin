...
fun test() {
    val x = <!OTHER_ERROR!>fun named1(x: Int): Int { return 1 }<!>
    ...
    val b = <!UNRESOLVED_REFERENCE!>(<!OTHER_ERROR!>fun named15(): Boolean { return true }<!>)()<!>

    baz(<!OTHER_ERROR!>fun named16(){}<!>)
}

fun bar() = <!OTHER_ERROR!>fun named() {}<!>
