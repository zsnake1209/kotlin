...
fun test(k: K) {
    ...
    // NI: TODO
    k.<!NI;OVERLOAD_RESOLUTION_AMBIGUITY!>bar<!> { <!NI;UNRESOLVED_REFERENCE!>it<!> <!NI;DEBUG_INFO_ELEMENT_WITH_ERROR_TYPE!>checkType<!> { <!NI;UNRESOLVED_REFERENCE!>_<!><Any>() }; "" } <!NI;DEBUG_INFO_ELEMENT_WITH_ERROR_TYPE!>checkType<!> { <!NI;UNRESOLVED_REFERENCE!>_<!><Int>() }
}