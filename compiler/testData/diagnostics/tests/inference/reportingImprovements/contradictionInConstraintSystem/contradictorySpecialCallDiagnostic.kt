...
; if), TYPE_MISMATCH!>if (true) "" else i<!>
    }
}
...
should be a supertype of: Nothing? \(for parameter 'thenBranch'\), Int \(for parameter 'thenBranch'\)
; if), TYPE_MISMATCH, TYPE_MISMATCH!>if (true) {
    when (2) {
        1 -> 1
        else -> null
    }
