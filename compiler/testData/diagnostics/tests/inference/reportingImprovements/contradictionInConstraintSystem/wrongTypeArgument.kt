...
fun <S, P> test1(a: Inv1<S>, b: P) {
    foo1(a, <!CONTRADICTION_IN_CONSTRAINT_SYSTEM(T; fun <T> foo1\(a: Inv1<T>, b: T\): Unit
...
fun <S, P> test3(a: S, b: Inv1<P>) {
    foo2(a, <!CONTRADICTION_IN_CONSTRAINT_SYSTEM(T; fun <T> foo2\(a: T, b: Inv1<T>\): Unit
should be a subtype of: P \(for parameter 'b'\)
...
fun <K, S> test4() {
    foo1(subCall<Inv1<K>>(), <!CONTRADICTION_IN_CONSTRAINT_SYSTEM(S; fun <S> subCall\(\): S
should be a subtype of: K \(for parameter 'b'\)
should be equal to: S
), TYPE_MISMATCH, TYPE_MISMATCH!>subCall<S>()<!>)
}
...
fun <T, S> test(a: Inv2<T, S>, v: S) {
    foo3(a, <!CONTRADICTION_IN_CONSTRAINT_SYSTEM(K; fun <K, V> foo3\(a: Inv2<K, V>, key: K\): Unit
