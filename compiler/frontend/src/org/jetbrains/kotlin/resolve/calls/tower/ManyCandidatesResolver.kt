...
abstract class ManyCandidatesResolver<D : CallableDescriptor>(
    ...
    fun resolveCandidates(resolutionCallbacks: KotlinResolutionCallbacks): List<ResolutionResultCallInfo<D>> {
        ...
        val hasOneSuccessfulAndOneErrorCandidate = if (resolvedCallsInfo.size > 1) {
            val hasErrors = resolvedCallsInfo.map { it.callResolutionResult.constraintSystem.errors.isNotEmpty() }
        ...
        if (hasOneSuccessfulAndOneErrorCandidate) {
            val goodCandidate = resolvedCallsInfo.first { it.callResolutionResult.constraintSystem.errors.isEmpty() }
            val badCandidate = resolvedCallsInfo.first { it.callResolutionResult.constraintSystem.errors.isNotEmpty() }

            listOf(goodCandidate, badCandidate).forEachIndexed { index, callInfo ->
                    ...
                    if (index == 1) {
                        for ((typeVariable, fixedType) in allCandidates[0].resolutionResult.constraintSystem.fixedTypeVariables) {
                            if (typeVariable in this.notFixedTypeVariables) {
                                val type = KotlinTypeFactory.simpleTypeWithNonTrivialMemberScope(
                                    Annotations.EMPTY, typeVariable as TypeConstructor, arguments = emptyList(),
