...
class LazyImportResolver(
    ...
    private val forceResolveImportDirective = storageManager.createMemoizedFunction { importForResolve: ImportWithDirective ->
        ...
        if (scope is LazyExplicitImportScope) {
            ...
            PlatformClassesMappedToKotlinChecker.checkPlatformClassesMappedToKotlin(
                    platformToKotlinClassMap, traceForImportResolve, importForResolve.import, allDescriptors
            )
        }
    ...
    private val forceResolveAllImportsTask: NotNullLazyValue<Unit> = storageManager.createLazyValue {
        val explicitClassImports = HashMultimap.create<Name, ImportWithDirective>()
        for (import in indexedImports.imports) {
            forceResolveImport(import)
            val scope = importedScopesProvider(ImportWithDirective(import))
        ...
        for ((importedName, import) in explicitClassImports.entries()) {
            if (importedName.asString().all { it == '_' }) {
                import.directive?.let { traceForImportResolve.report(Errors.UNDERSCORE_IS_RESERVED.on(it)) }
            }
        }
        for (importedName in explicitClassImports.keySet()) {
            ...
            if (imports.size > 1) {
                imports.forEach { import ->
                    import.directive?.let { traceForImportResolve.report(Errors.CONFLICTING_IMPORT.on(it, importedName.asString())) }
                }
            }
        }
    }
