        ...
        newIrFunction.body = builder.irBlockBody(newIrFunction) {
            ...
            when (irFunction) {
                is IrConstructor -> +irDelegatingConstructorCall(irFunction).apply {
                    ...
                    // apparently do have type parameters and dispatch receivers:
...
open class DefaultParameterInjector(
    ...
    private fun parametersForCall(expression: IrFunctionAccessExpression): Pair<IrFunctionSymbol, List<IrExpression?>>? {
        ...
        val realArguments = declaration.valueParameters.size
        val maskValues = IntArray((declaration.valueParameters.size + 31) / 32)
        assert((stubFunction.valueParameters.size - realArguments - maskValues.size) in listOf(0, 1)) {
            "argument count mismatch: expected $realArguments arguments + ${maskValues.size} masks + optional handler/marker, " +
        ...
        return stubFunction.symbol to stubFunction.valueParameters.mapIndexed { i, parameter ->
            when {
                i >= realArguments + maskValues.size -> IrConstImpl.constNull(startOffset, endOffset, parameter.type)
                i >= realArguments -> IrConstImpl.int(startOffset, endOffset, parameter.type, maskValues[i - realArguments])
                else -> {
