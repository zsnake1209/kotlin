...
) : AbstractFakeOverrideBuilder
{
    ...
    val propertyOverriddenSymbols = mutableMapOf<IrOverridableMember, List<IrSymbol>>()
    private val irOverridingUtil = IrOverridingUtil(irBuiltIns, this)

    fun fakeOverrideMember(superType: IrType, member: IrDeclaration, clazz: IrClass, newModality: Modality? = null, newVisibility: Visibility? =  null): IrOverridableMember {
        ...
        require(member is IrOverridableMember)
        val deepCopyFakeOverride = copier.copy(member) as IrDeclaration
        require(deepCopyFakeOverride is IrOverridableMember)
    ...
    fun redelegateFakeOverride(fake: IrDeclaration) {
        when (fake) {
            is IrSimpleFunction -> {
    ...
    private fun redelegateFakeOverrides(fakeOverrides: List<IrDeclaration>) =
        fakeOverrides.forEach { redelegateFakeOverride(it) }
