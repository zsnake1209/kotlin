...
class WasmSymbols(
    ...
    override val coroutineContextGetter by lazy {
        context.bodilessBuiltInsPackageFragment.addFunction {
    ...
    fun wasmString(simpleType: SimpleType): String = with(context.irBuiltIns) {
        when (simpleType) {
...
//
//        lessFunByOperandType.map { it.value to getInternalFunction("wasm_${wasmString(it.key)}_lt") }
//        lessOrEqualFunByOperandType.map { it.value to getInternalFunction("wasm_${wasmString(it.key)}_lt") }
    }

    val lessThanFunctions = mapOf(
        context.irBuiltIns.booleanType to getInternalFunction("wasm_i32_eq"),
        context.irBuiltIns.byteType to getInternalFunction("wasm_i32_eq"),
        context.irBuiltIns.shortType to getInternalFunction("wasm_i32_eq"),
        context.irBuiltIns.charType to getInternalFunction("wasm_i32_eq"),
