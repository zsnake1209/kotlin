...
class IrModuleToWasm(private val backendContext: WasmBackendContext) {
    fun generateModule(module: IrModuleFragment): WasmCompilerResult {
        ...
        val context = WasmContext(nameTable, backendContext)
        val irDeclarations = module.files.flatMap { it.declarations }
        val wasmDeclarations = irDeclarations.mapNotNull { it.accept(IrDeclarationToWasmTransformer(), context) }
    ...
    private fun generateExports(module: IrModuleFragment, context: WasmContext): List<WasmExport> {
    ...
    private fun generateExport(declaration: IrDeclaration, context: WasmContext): WasmExport? {
    ...
    private fun IrFunction.isExported(): Boolean =
        fqNameWhenAvailable in backendContext.additionalExportedDeclarations
}