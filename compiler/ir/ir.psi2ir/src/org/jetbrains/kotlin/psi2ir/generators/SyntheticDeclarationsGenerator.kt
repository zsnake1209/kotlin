...
class SyntheticDeclarationsGenerator(private val irModuleFragment: IrModuleFragment, private val context: GeneratorContext, private val pluginProviders: Collection<IrExtensionGenerator>) : DeclarationDescriptorVisitor<Unit, IrDeclarationContainer?> {
    ...
    private fun MemberScope.declareMemberScope(container: IrDeclarationContainer) {
        val memberScopeDesc = DescriptorUtils.getAllDescriptors(this)
        memberScopeDesc.forEach { it.accept(this@SyntheticDeclarationsGenerator, container) }
    }
    ...
    override fun visitPackageFragmentDescriptor(descriptor: PackageFragmentDescriptor, data: IrDeclarationContainer?) {
        if (descriptor !in visited) {
            ...
            val packageFragment =
                irModuleFragment.files.first { it.symbol.descriptor == descriptor }
            descriptor.getMemberScope().declareMemberScope(packageFragment)
        }
    }
    ...
    override fun visitClassDescriptor(descriptor: ClassDescriptor, data: IrDeclarationContainer?) {
        ...
        if (DescriptorUtils.isEnumEntry(descriptor)) {
            ...
            val irClass = symbolTable.declareClassIfNotExists(descriptor) {
                createClassStub(descriptor, it).insertDeclaration(data)
            }

            symbolTable.withScope(descriptor) {
                descriptor.unsubstitutedMemberScope.declareMemberScope(irClass)
                descriptor.constructors.forEach { it.accept(this@SyntheticDeclarationsGenerator, irClass) }
                descriptor.companionObjectDescriptor?.accept(this@SyntheticDeclarationsGenerator, irClass)
                descriptor.sealedSubclasses.forEach { it.accept(this@SyntheticDeclarationsGenerator, irClass) }
            }
        }
    }
