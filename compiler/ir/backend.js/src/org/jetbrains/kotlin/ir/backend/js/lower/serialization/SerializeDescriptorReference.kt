...
class DescriptorReferenceSerializer(val declarationTable: DeclarationTable) {
    ...
    fun serializeDescriptorReference(declaration: IrDeclaration): KonanIr.DescriptorReference? {
        ...
        val realDeclaration = if (isFakeOverride) {
            when (declaration) {
                is IrSimpleFunction -> declaration.collectRealOverrides().first()
                is IrField -> declaration.correspondingProperty?.getter?.collectRealOverrides()?.first()?.correspondingProperty?.backingField!!
                is IrProperty -> declaration.getter?.collectRealOverrides()?.first()?.correspondingProperty!!
