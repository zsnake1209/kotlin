        ...
        irFile.transformChildren(object : IrElementTransformer<IrDeclarationParent> {
            ...
            private fun mergeTypes(types: List<IrType>): IrType {
                ...
                return types.firstOrNull { it is IrDynamicType } ?: run {
                    ...
                    val superClassifier =
                        types.map { (it as IrSimpleType).classifier }.commonSuperclass(context.classifierEqualityChecker).also {
                            assert(it.isSubtypeOfClass(context.irBuiltIns.throwableClass, context.classifierEqualityChecker))
                        }
