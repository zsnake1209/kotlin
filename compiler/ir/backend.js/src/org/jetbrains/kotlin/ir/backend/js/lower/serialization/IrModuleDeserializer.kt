...
import org.jetbrains.kotlin.ir.backend.js.lower.serialization.KonanIr.IrOperation.OperationCase.*
import org.jetbrains.kotlin.ir.backend.js.lower.serialization.KonanIr.IrConst.ValueCase.*
import org.jetbrains.kotlin.ir.backend.js.lower.serialization.KonanIr.IrTypeArgument.KindCase.*
import org.jetbrains.kotlin.ir.backend.js.lower.serialization.KonanIr.IrType.KindCase.*
import org.jetbrains.kotlin.ir.backend.js.lower.serialization.KonanIr.IrDeclarator.DeclaratorCase.*
...
abstract class IrModuleDeserializer(
    ...
    private fun deserializeStatement(proto: KonanIr.IrStatement): IrElement {
        ...
        val element = when (proto.statementCase) {
            KonanIr.IrStatement.StatementCase.BLOCK_BODY //proto.hasBlockBody()
            -> deserializeBlockBody(proto.blockBody, start, end)
            KonanIr.IrStatement.StatementCase.BRANCH //proto.hasBranch()
            -> deserializeBranch(proto.branch, start, end)
            KonanIr.IrStatement.StatementCase.CATCH //proto.hasCatch()
            -> deserializeCatch(proto.catch, start, end)
            KonanIr.IrStatement.StatementCase.DECLARATION // proto.hasDeclaration()
            -> deserializeDeclaration(proto.declaration, null) // TODO: we need a proper parent here.
            KonanIr.IrStatement.StatementCase.EXPRESSION // proto.hasExpression()
            -> deserializeExpression(proto.expression)
            KonanIr.IrStatement.StatementCase.SYNTHETIC_BODY // proto.hasSyntheticBody()
...
//        logger.log { "### Deserialized statement: ${ir2string(element)}" }
    ...
    private fun deserializeVarargElement(element: KonanIr.IrVarargElement): IrVarargElement {
        return when (element.varargElementCase) {
            KonanIr.IrVarargElement.VarargElementCase.EXPRESSION
            -> deserializeExpression(element.expression)
            KonanIr.IrVarargElement.VarargElementCase.SPREAD_ELEMENT
    ...
    private fun deserializeConst(proto: KonanIr.IrConst, start: Int, end: Int, type: IrType): IrExpression =
        when (proto.valueCase) {
            ...
            else -> error("Unreachable execution")
        }
