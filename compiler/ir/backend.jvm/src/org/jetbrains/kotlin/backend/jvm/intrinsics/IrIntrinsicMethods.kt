...
class IrIntrinsicMethods(val irBuiltIns: IrBuiltIns, val symbols: JvmSymbols) {
    ...
    private val intrinsicsMap = (
            listOf(
                ...
                irBuiltIns.throwNpeSymbol.toKey()!! to ThrowNPE
                    ...
                    binaryFunForPrimitives("equals") { type ->
    ...
    private fun binaryFunForPrimitives(name: String, getIntrinsic: (PrimitiveType) -> IntrinsicMethod): List<Pair<Key, IntrinsicMethod>> =
        PrimitiveType.values().flatMap { type ->
            methodWithArity(irBuiltIns.primitiveTypeToIrType[type]!!.classOrNull!!, name, 1, getIntrinsic(type))
        }
