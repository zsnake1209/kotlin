...
class JvmSerializerExtension(private val bindings: JvmSerializationBindings, state: GenerationState) : SerializerExtension() {
    ...
    override fun serializeConstructor(descriptor: ConstructorDescriptor, proto: ProtoBuf.Constructor.Builder, serializer: DescriptorSerializer) {
    ...
    override fun serializeFunction(descriptor: FunctionDescriptor, proto: ProtoBuf.Function.Builder, serializer: DescriptorSerializer) {
        val method = bindings.get(METHOD_FOR_FUNCTION, descriptor)
        if (method != null) {
            val signature = SignatureSerializer().methodSignature(descriptor, method)
            if (signature != null) {
