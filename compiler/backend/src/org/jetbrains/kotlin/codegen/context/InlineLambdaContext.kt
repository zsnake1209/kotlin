...
class InlineLambdaContext(
    ...
    private fun isSuspendIntrinsicParameter(): Boolean {
        ...
        var psiElement = contextDescriptor.source.getPsi()
        while (psiElement != null && psiElement !is KtCallExpression) {
            psiElement = psiElement.parent
        }
        if (psiElement == null) return false
        val call = state.bindingContext[BindingContext.CALL, (psiElement as KtCallExpression).calleeExpression] ?: return false
        val resolvedCall = state.bindingContext[BindingContext.RESOLVED_CALL, call] ?: return false
