...
object BuiltinSpecialBridgesUtil {
    ...
    fun <Signature : Any> generateBridgesForBuiltinSpecial(
        ...
        val functionHandle = DescriptorBasedFunctionHandleForJvm(function, jvmTarget)
        val fake = !functionHandle.isDeclaration
        val overriddenBuiltin = function.getOverriddenBuiltinReflectingJvmDescriptor()!!

        val reachableDeclarations = findAllReachableDeclarations(function, jvmTarget)
        ...
        if (fake) {
            for (overridden in function.overriddenDescriptors.map { it.original }) {
                if (!DescriptorBasedFunctionHandleForJvm(overridden, jvmTarget).isAbstract) {
        ...
        val superImplementationDescriptor =
            if (specialBridge != null && fake && !functionHandle.isAbstract)
                findSuperImplementationForStubDelegation(function, jvmTarget, signatureByDescriptor)
            else
                null
...
private fun <Signature> findSuperImplementationForStubDelegation(
    ...
    val implementation = findConcreteSuperDeclaration(DescriptorBasedFunctionHandleForJvm(function, jvmTarget)) ?: return null
