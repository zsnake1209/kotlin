...
public class ZipHandler extends ZipHandlerBase {
    ...
    private static final FileAccessorCache<ZipHandler, ZipFile> ourZipFileFileAccessorCache =
            new FileAccessorCache<ZipHandler, ZipFile>(20, 10) {
                @NotNull
                @Override
    ...
    @Contract("true -> !null")
    protected FileAccessorCache.Handle<ZipFile> getCachedZipFileHandle_(boolean createIfNeeded) throws IOException {
        try {
            FileAccessorCache.Handle<ZipFile> handle =
                    createIfNeeded ? ourZipFileFileAccessorCache.get(this) : ourZipFileFileAccessorCache.getIfCached(this);
    ...
    protected FileAccessorCache.Handle<ZipFile> getCachedZipFileHandle(boolean createIfNeeded) throws IOException {
        if (!initMessage) {
            System.out.println("Using hacked ZipHandler");
