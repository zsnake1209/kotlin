...
class FirCorrespondingSupertypesCache(private val session: FirSession) {
    ...
    fun getCorrespondingSupertypes(
        ...
        val symbol = with(context) {
            type.typeConstructor() as? ConeClassLikeSymbol ?: return null
        }
        if (symbol == supertypeConstructor) return listOf(captureType(type))

        if (symbol !in cache) {
            cache[symbol] = computeSupertypesMap(symbol)
        }
    ...
    private fun computeSupertypesMap(
        ...
        val subtypeFirClass = subtypeSymbol.toFirClassLike() ?: return null

        val defaultType = subtypeSymbol.toLookupTag().constructClassType(
    ...
    private fun computeSupertypePolicyAndPutInMap(
        ...
        require(supertype is ConeClassLikeType) {
            "Supertypes should not be computed for non-classes types, but [$supertype](${supertype::class}) was found"
        }

        val supertypeSymbol = supertype.lookupTag.toSymbol(session)
        if (supertypeSymbol is ConeClassLikeSymbol && supertypeSymbol != subtypeSymbol) {
            resultingMap[supertypeSymbol] = listOf(supertype)
        }

        return when {
            with(context) { supertype.argumentsCount() } == 0 -> {
                AbstractTypeCheckerContext.SupertypesPolicy.LowerIfFlexible
            }
            else -> {
                context.substitutionSupertypePolicy(supertype)
            }
        }
    }
}
