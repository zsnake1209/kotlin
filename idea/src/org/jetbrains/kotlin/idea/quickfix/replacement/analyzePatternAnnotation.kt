...
fun ResolvablePattern.toCodeToInline(
    symbolDescriptor: CallableDescriptor,
    resolutionFacade: ResolutionFacade,
    reformat: Boolean
): CodeToInline {
    return CodeToInlineBuilder(symbolDescriptor.toOriginal(), resolutionFacade).prepareCodeToInline(pattern, emptyList(), analyze, reformat)
}

fun PatternAnnotationData.analyzeAsExpression(
    symbolDescriptor: CallableDescriptor,
    resolutionFacade: ResolutionFacade
): ResolvablePattern? {
    ...
    return ResolvablePattern(expression,
                             analyze = { expression.analyzeInContext(scope, expressionTypingServices = expressionTypingServices) })
}
