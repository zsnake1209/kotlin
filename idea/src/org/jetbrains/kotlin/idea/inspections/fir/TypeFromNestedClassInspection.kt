...
class TypeFromNestedClassInspection : AbstractKotlinInspection() {
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        return namedFunctionVisitor(fun(function: KtNamedFunction) {
            ...
            val firFunction = function.getOrBuildFir(function.firResolveState()) as FirSimpleFunction
            val firFunctionId = (firFunction.symbol as? FirFunctionSymbol)?.callableId ?: return

            val coneType = firFunction.returnTypeRef.coneTypeSafe<ConeClassLikeType>() ?: return
            val typeId = (coneType.lookupTag.toSymbol(firFunction.session) as? FirClassSymbol)?.classId ?: return
