...
class UnusedSymbolInspection : AbstractKotlinInspection() {
    companion object {
        ...
        fun listOfPropertyAccessorNames(property: KtProperty): List<String> {
            ...
            val getterName = property.getter?.let { getCustomAccessorName(it) } ?: JvmAbi.getterName(propertyName)
            if (getterName != null)
                accessors.add(getterName)
            if (property.isVar) {
                val setterName = property.setter?.let { getCustomAccessorName(it) } ?: JvmAbi.setterName(propertyName)
                accessors.add(setterName)
            }
            return accessors
        }
