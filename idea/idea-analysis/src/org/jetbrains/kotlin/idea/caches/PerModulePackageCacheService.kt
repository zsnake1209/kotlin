...
class KotlinPackageStatementPsiTreeChangePreprocessor(private val project: Project) : PsiTreeChangePreprocessor {
    override fun treeChanged(event: PsiTreeChangeEventImpl) {
        ...
        if (eFile == null) LOG.debugIfEnabled("Got PsiEvent: $event without file", true)
        ...
        when (event.code) {
            ...
            PsiTreeChangeEventImpl.PsiEventType.CHILD_REMOVED -> {
                val child = event.child ?: run {
                    LOG.debugIfEnabled("Got PsiEvent: $event without child", true)
                    return
                }
            ...
            PsiTreeChangeEventImpl.PsiEventType.CHILDREN_CHANGED -> {
                val parent = event.parent ?: run {
                    LOG.debugIfEnabled("Got PsiEvent: $event without parent", true)
                    return
                }
...
class PerModulePackageCacheService(private val project: Project) {
    ...
    private fun invalidateCacheForModuleSourceInfo(moduleSourceInfo: ModuleSourceInfo) {
        LOG.debugIfEnabled("Invalidated cache for $moduleSourceInfo", false)
        ...
        if (pendingVFileChanges.size + pendingKtFileChanges.size >= FULL_DROP_THRESHOLD) {
                ...
                if (vfile.isDirectory || !vfile.isValid) {
                    for ((module, data) in cache) {
                        ...
                        if (sourceRootUrls.any { url ->
                            ...
                            LOG.debugIfEnabled("Invalidated cache for $module")
                            data.clear()
                        }
                    }
                } else {
                    ...
                    if (infoByVirtualFile == null || infoByVirtualFile !is ModuleSourceInfo) {
                        LOG.debugIfEnabled("Skip $vfile as it has mismatched ModuleInfo=$infoByVirtualFile")
                    }
            ...
            pendingKtFileChanges.processPending { file ->
                if (file.virtualFile != null && file.virtualFile !in projectScope) {
                    LOG.debugIfEnabled("Skip $file without vFile, or not in scope: ${file.virtualFile?.let { it !in projectScope }}")
                ...
                if (nullableModuleInfo == null || nullableModuleInfo !is ModuleSourceInfo) {
                    LOG.debugIfEnabled("Skip $file as it has mismatched ModuleInfo=$nullableModuleInfo")
                }
    ...
    fun packageExists(packageFqName: FqName, moduleInfo: ModuleSourceInfo): Boolean {
        ...
        return cacheForCurrentModuleInfo.getOrPut(packageFqName) {
            ...
            LOG.debugIfEnabled("Computed cache value for $packageFqName in $moduleInfo is $packageExists")
            packageExists
        }
    }
...
private val DEBUG_LOG_ENABLE: Boolean get() = run { System.getProperty("debug.PerModulePackageCache", "false") == "true" }

private fun Logger.debugIfEnabled(message: String, withCurrentTrace: Boolean = false) {
    if (ApplicationManager.getApplication().isUnitTestMode && DEBUG_LOG_ENABLE) {
        if (withCurrentTrace) {
